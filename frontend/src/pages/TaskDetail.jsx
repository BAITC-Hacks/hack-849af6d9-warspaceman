...
export default function TaskDetail({ task, taskId, role = 'business' }) {
  const id = taskId || task?.id; const [proposals, setProposals] = useState([]); const [teams, setTeams] = useState([]); const [rating, setRating] = useState(null); const [form, setForm] = useState({ team_id: '', idea: '', plan: '', deadline: '', link: '' }); const [loading, setLoading] = useState(false); const [ratingLoading, setRatingLoading] = useState(true); const [teamsLoading, setTeamsLoading] = useState(true); const [submitting, setSubmitting] = useState(false); const [pendingAction, setPendingAction] = useState(null); const [success, setSuccess] = useState(''); const [error, setError] = useState(''); const [proposalError, setProposalError] = useState(''); const [teamsError, setTeamsError] = useState('')
  useEffect(() => {
    if (!id) return
    let active = true
  ...
  useEffect(() => {
    ...
    listTeams().then((items) => { if (active) setTeams(items) }).catch((reason) => { if (active) setTeamsError(reason.message) }).finally(() => { if (active) setTeamsLoading(false) })
    return () => { active = false }
  }, [id])
